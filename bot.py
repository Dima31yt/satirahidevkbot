...
token = "ce2dbe2afa1d0e75534d3db448a19b70d1dfaf6970d38ed569539cc59fa4bf56750ab422a11e2eacd03c7"
vk_session_u = vk_api.VkApi(token=token)
...
def send_me(id, text, media):
	vk_session.method('messages.send', {'user_id' : id, 'message' : text, "attachment": media, 'random_id' : 0})
...
def callback_add(name, type):
	keyboard_1 = VkKeyboard(**settings) #Настройки кнопки
	keyboard_1.add_callback_button(label=name, color=VkKeyboardColor.POSITIVE, payload={"type": str(type)}) #Кнопка
...
def note_plus(number_k):
	with open('notes.json', encoding='utf-8') as f:
		data = json.load(f)

	notes_key = VkKeyboard(**settings)
	...
	while b != 5:
		if data["note-1"] == "None":
			...
			a += 1
			b += 1
			notes_key.add_callback_button(label="Заметка 1", color=VkKeyboardColor.POSITIVE, payload={"type": "note-1"})
			notes_key.add_line()
		...
		if data["note-2"] == "None":
			...
			a += 1
			b += 1
			notes_key.add_callback_button(label="Заметка 2", color=VkKeyboardColor.POSITIVE, payload={"type": "note-2"})
			notes_key.add_line()
		...
		if data["note-3"] == "None":
			...
			a += 1
			b += 1
			notes_key.add_callback_button(label="Заметка 3", color=VkKeyboardColor.POSITIVE, payload={"type": "note-3"})
			notes_key.add_line()
		...
		if data["note-4"] == "None":
			...
			a += 1
			b += 1
			notes_key.add_callback_button(label="Заметка 4", color=VkKeyboardColor.POSITIVE, payload={"type": "note-4"})
			notes_key.add_line()
		...
		if data["note-5"] == "None":
			...
			b += 1
			a += 1
			notes_key.add_callback_button(label="Заметка 5", color=VkKeyboardColor.POSITIVE, payload={"type": "note-5"})
			notes_key.add_line()

	if a == 0:
		sender(id, "Заметок нет")

	else:
		notes_key.add_callback_button(label="Стр 1/2", color=VkKeyboardColor.PRIMARY, payload={"type": "str_note_1-2"})
		if number_k == 0:
			send_key(id, "Выберете заметку:", notes_key.get_keyboard())
		if number_k == 1:
			vk.messages.edit(peer_id=event.obj.peer_id, message= "Выберете заметку:", conversation_message_id=event.obj.conversation_message_id, keyboard=notes_key.get_keyboard())
...
for event in longpoll.listen():
	if event.type == VkBotEventType.MESSAGE_EVENT:
		...
		if event.object.payload.get('type') == 'str_note_1-2':
			str_note_1_2 = VkKeyboard(**settings)
			str_note_1_2.add_callback_button(label="Добавить заметку", color=VkKeyboardColor.POSITIVE, payload={"type": "show_snackbar", "text": "Напишите: заметка+ + номер + заметка"})
			str_note_1_2.add_line()
			str_note_1_2.add_callback_button(label="Удалить заметку", color=VkKeyboardColor.NEGATIVE, payload={"type": "show_snackbar", "text": "Напишите: заметка- + номер"})
			str_note_1_2.add_line()
			str_note_1_2.add_callback_button(label="Стр 2/2", color=VkKeyboardColor.PRIMARY, payload={"type": "str_note_2-2"})

			vk.messages.edit(peer_id=event.obj.peer_id, message="Страница 2", conversation_message_id=event.obj.conversation_message_id, keyboard=str_note_1_2.get_keyboard())

		if event.object.payload.get('type') == 'str_note_2-2':
			note_plus(1)

		if event.object.payload.get('type') == 'note-1':
			with open('notes.json', encoding='utf-8') as f:
				data = json.load(f)

			sender(id, "Заметка 1:<br>" + data["note-1"])
			
		if event.object.payload.get('type') == 'note-2':
			with open('notes.json', encoding='utf-8') as f:
				data = json.load(f)

			sender(id, "Заметка 2:<br>" + data["note-2"])
			
		if event.object.payload.get('type') == 'note-3':
			with open('notes.json', encoding='utf-8') as f:
				data = json.load(f)

			sender(id, "Заметка 3:<br>" + data["note-3"])
			
		if event.object.payload.get('type') == 'note-4':
			with open('notes.json', encoding='utf-8') as f:
				data = json.load(f)

			sender(id, "Заметка 4:<br>" + data["note-4"])
			
		if event.object.payload.get('type') == 'note-5':
			with open('notes.json', encoding='utf-8') as f:
				data = json.load(f)

			sender(id, "Заметка 5:<br>" + data["note-5"])
		...
		if event.object.payload.get('type') == "command_ls":
			sender_me(id, "Это всё что доступно на данный момент:<br>Фото поиск + запрос<br>Видео поиск + запрос")
	...
	if event.type == VkBotEventType.MESSAGE_NEW:				#Обработка команд
		if event.from_user:		#Если написали в ЛС
			...
			if msg in ["начать", "привет", "ей", "бот"]:
				...
				start_k.add_line()
				start_k.add_callback_button(label='Команды в ЛС', color=VkKeyboardColor.POSITIVE, payload={"type": "command_ls"})
				send_key(id, 'Приветствую тебя', start_k.get_keyboard())

			if "фото поиск" in event.obj.text.lower():
				try:
					photo_s_l = msg.split(" ", 2)
					photo_s = vks.photos.search(q=photo_s_l[2],sort=1, count=1000)
					...
					send_me(id, '', photo_s_f)
				except:
					sender_me(id, "По вашему запросу ничего не найдено")
			...
			try:
				...
				if "-" in str(user):		#Защита от команд бота
					...
					try:		#Создание профиля если он не создан
						...
						a_dict = ({user: {"first_name": str(user_n_r_1), "last_name": user_n_r_2, "full_name": user_n_r_1 + ' ' + user_n_r_2, "id_name": "[id" + str(user) + "|" + user_n_r_1 + " " + user_n_r_2 + "]", "money": 0, "role": "Участник", "warn": 0, "nick": "none", "kid": "none", "me_msg": 0, "msg_money": 0},})
					...
					with open('admins_int.txt', 'r', encoding='utf-8') as f:
						data = f.read()

					admin_from = ["400484262", "360873634"] + data.split("\n")
					admin_true = ["400484262", "360873634"]
					...
					if data[str(user)]["msg_money"] == 10:
						data[str(user)]["msg_money"] = 0
						data[str(user)]["money"] += 1

					else:
						data[str(user)]["msg_money"] += 1
					...
					if "сатира обнять" in event.obj.text.lower():
						...
						if int(hug_sex) == 2:
							hug_sex_s = 'обнял'
						hug_w_u_n = vk.users.get(user_ids=hug_w_n, name_case="acc")
					...
					if "сатира поцеловать" in event.obj.text.lower():
						...
						kiss_w_u_n = vk.users.get(user_ids=kiss_w_n, name_case="acc")
					...
					if "сатира отшлëпать" in event.obj.text.lower():
						...
						if int(slap_sex) == 2:
							slap_sex_s = 'отшлëпал'
						slap_w_u_n = vk.users.get(user_ids=slap_w_n, name_case="acc")
					...
					if "сатира трахнуть" in event.obj.text.lower() or  "сатира выебать" in event.obj.text.lower():
						if (len(te2xt) == 2):
							fuck_w_n = reply["from_id"]

						if (len(te2xt) == 3):
							fuck1 = msg.split(' ')[2]
							fuck_w_n = get_user(fuck1)

						###Идентификатор альбома ----- 278597872
						album_fuck = vk_u.photos.get(owner_id=-201980948, album_id=278597872)
						...
						fuck_w_u_n = vk.users.get(user_ids=fuck_w_n, name_case="acc")
					...
					if "сатира пойти выпить" in event.obj.text.lower() or "сатира выпить" in event.obj.text.lower():

						###Идентификатор альбома ----- 278594266
						album_brew = vk_u.photos.get(owner_id=-201980948, album_id=278594266)
						brew_p1 = random.choice(album_brew["items"])
						brew_p = 'photo' + str(brew_p1["owner_id"]) + "_" + str(brew_p1["id"])

						if (len(te2xt) == 3) and msg == "сатира пойти выпить" or (len(te2xt) == 2) and msg == "сатира выпить":
							try:
								brew_w_n = reply["from_id"]

								brew_u_n_p = vk.users.get(user_ids=user, fields="sex")
								brew_u_n_1 = brew_u_n_p[0]["first_name"]
								brew_u_n_2 = brew_u_n_p[0]["last_name"]
								brew_sex = brew_u_n_p[0]["sex"]
								brew_u_n_l = brew_u_n_1 + ' ' + brew_u_n_2
								brew_u_n_f = '[id' + str(user) + '|' + brew_u_n_l + ']'

								brew_u_n_p_2 = vk.users.get(user_ids=brew_w_n, name_case='ins')
								brew_u_n_1_2 = brew_u_n_p_2[0]["first_name"]
								brew_u_n_2_2 = brew_u_n_p_2[0]["last_name"]
								brew_u_n_l_2 = brew_u_n_1_2 + ' ' + brew_u_n_2_2
								brew_u_n_f_2 = '[id' + str(brew_w_n) + '|' + brew_u_n_l_2 + ']'

								if int(brew_sex) == 1:
									brew_sex_s = random.choice([' пошла выпить с ', ' бухает с '])
								if int(brew_sex) == 2:
									brew_sex_s = random.choice([' пошёл выпить с ', ' бухает с '])

								send(id, brew_u_n_f + brew_sex_s + brew_u_n_f_2, brew_p)
								...
								if int(brew_sex) == 1:
									brew_sex_s = random.choice([' пошла выпить', ' бухает'])
								if int(brew_sex) == 2:
									brew_sex_s = random.choice([' пошёл выпить', ' бухает'])
								
								send(id, brew_u_n_f + brew_sex_s, brew_p)

						if (len(te2xt) == 4) and "сатира пойти выпить" in event.obj.text.lower() or (len(te2xt) == 3) and "сатира выпить" in event.obj.text.lower():
							if (len(te2xt) == 4) and "сатира пойти выпить" in event.obj.text.lower():
								brew1 = msg.replace('сатира пойти выпить ', '')
								brew_w_n = get_user(brew1)
							...
							if (len(te2xt) == 3) and "сатира выпить" in event.obj.text.lower():
								brew1 = msg.replace('сатира выпить ', '')
								brew_w_n = get_user(brew1)
					...
					if "сатира кусь" in event.obj.text.lower():
						if (len(te2xt) == 2):
							kuss_w_n = reply["from_id"]

						if (len(te2xt) == 3):
							kuss1 = msg.replace('сатира кусь ', '')
							kuss_w_n = get_user(kuss1)

						###Идентификатор альбома ----- 278594132
						album_kuss = vk_u.photos.get(owner_id=-201980948, album_id=278594132)
						kuss_p1 = random.choice(album_kuss["items"])
						kuss_p = 'photo' + str(kuss_p1["owner_id"]) + "_" + str(kuss_p1["id"])

						kuss_n_u = vk.users.get(user_ids=user)
						kuss_n_1 = kuss_n_u[0]["first_name"]
						kuss_n_2 = kuss_n_u[0]["last_name"]
						kuss_n_f = kuss_n_1 + ' ' + kuss_n_2
						kuss_sex1 = vk_session.method('users.get', {'user_ids' : str(user), 'fields' : 'sex', 'name_case' : 'nom'})
						kuss_sex0 = kuss_sex1[0]
						kuss_sex = json.dumps(kuss_sex0['sex'])
						kuss_sex_s = ''
						if int(kuss_sex) == 1:
							kuss_sex_s = 'укусила'
						if int(kuss_sex) == 2:
							kuss_sex_s = 'укусил'
						kuss_w_u_n = vk.users.get(user_ids=kuss_w_n, name_case="acc")
						kuss_w_1 = kuss_w_u_n[0]["first_name"]
						kuss_w_2 = kuss_w_u_n[0]["last_name"]
						kuss_w_n_f = kuss_w_1 + ' ' + kuss_w_2
						send(id, '[id' + str(user) + '|' + kuss_n_f + '] ' + kuss_sex_s + ' [id' + str(kuss_w_n) + '|' + kuss_w_n_f + ']', kuss_p)
					...
					if "сатира сжечь" in event.obj.text.lower():
						...
						if int(fire_sex) == 2:
							fire_sex_s = 'сжёг'
						fire_w_u_n = vk.users.get(user_ids=fire_w_n, name_case="acc")
					...
					if msg in ['сатира команды', 'сатира помощь', 'сатира помошь', 'сатира помощ']:
						if str(user) in admin_from:
							with open('settings.json', encoding='utf-8') as f:
								data = json.load(f)
							sender(id, f''+ data["commands"] + data["commands_admins"])
					...
					if msg in ['сатира админы']:
						...
						a = -1
						admin_split = data.split("\n")
						admin__full = ""
						try:
							while True:
								a += 1
								admin_split_who = admin_split[a]
								admin__u = vk.users.get(user_ids=admin_split_who)
								admin__1 = admin__u[0]["first_name"]
								admin__2 = admin__u[0]["last_name"]
								admin_2_f = admin__1 + ' ' + admin__2
								admin__f = '[id' + str(admin_split_who) + '|' + admin_2_f + ']'
								admin__full += "<br>" + admin__f
						except:	
							sender(id, f'Создатели: <br>[id360873634|Александр Ртищев]<br>[id400484262|Дмитрий Князев]<br>Админы этой беседы:' + admin__full)
					...
					if "сатира ударить" in event.obj.text.lower():
						...
						if int(hit_sex) == 2:
							hit_sex_s = 'ударил'
						hit_w_u_n = vk.users.get(user_ids=hit_w_n, name_case="acc")
					...
					if "сатира кража" in event.obj.text.lower():
						...
						steal_w_u_n = vk.users.get(user_ids=steal_w_n, name_case="acc")
					...
					if "сатира админ+" in event.obj.text.lower():
						admin_p_l = msg.split(' ')[2]
						admin_p_id = get_user(admin_p_l)
						if str(user) in admin_true:
							with open('admins_int.txt', 'r', encoding='utf-8') as f:
								data = f.read()
							
							data += str('\n' + str(admin_p_id))

							with open('admins_int.txt', 'w', encoding='utf-8') as f:
								f.write(data)

							with open('users.json', encoding='utf-8') as f:
								data = json.load(f)

							data[str(admin_p_id)]["role"] = "Админ"
							...
							admin_p_1 = admin_p_u[0]["first_name"]
							admin_p_2 = admin_p_u[0]["last_name"]
							admin_p2_f = admin_p_1 + ' ' + admin_p_2
							admin_p_f = '[id' + str(admin_p_id) + '|' + admin_p2_f + ']'
							
							sender(id, "Пользователь " + admin_p_f + " был добавлен в список админов")

						else:
							sender(id, f'Вам отказано в доступе')

					if "сатира админ-" in event.obj.text.lower():
						admin_m_l = msg.split(' ')[2]
						admin_m_id = get_user(admin_m_l)
						if str(user) in admin_true:
							with open('admins_int.txt', 'r', encoding='utf-8') as f:
								data = f.read()
							...
							data[str(admin_p_id)]["role"] = "Участник"
							...
							admin_m_1 = admin_m_u[0]["first_name"]
							admin_m_2 = admin_m_u[0]["last_name"]
							admin_m2_f = admin_m_1 + ' ' + admin_m_2
							admin_m_f = '[id' + str(admin_m_id) + '|' + admin_m2_f + ']'
							
							sender(id, "Пользователь " + admin_m_f + " был разжалован")
					...
					if "сатира профиль" in event.obj.text.lower():
						if (len(te2xt) == 2) and msg == "сатира профиль":
							with open('users.json', encoding='utf-8') as f:
								prof = json.load(f)
						...
						if (len(te2xt) == 3):
							try:
								chprof = msg.split(" ")
								drcht9 = chprof[2].split("|")[0].replace("[id", "")
								with open('users.json', encoding='utf-8') as f:
									prof = json.load(f)
						...
						if str(user) in admin_from:
							if (len(te2xt) == 2):
								...
								if art_c[1] == 'поцеловать':
									...
									filename = "\\app\\files\\" + os.path.basename(photo).split('?')[0]
								...
								if art_c[1] == 'кража':
									...
									upload = VkUpload(vk_session_u)
									ph = [filename]
									photo_list = upload.photo(photos=ph, album_id=277598193, group_id=201980948)
									sender(id, "Готово<br>Айди: " + str(photo_list[0]["id"]))
								...
								if art_c[1] == 'ударить':
									...
									upload = VkUpload(vk_session_u)
									ph = [filename]
									photo_list = upload.photo(photos=ph, album_id=277598253, group_id=201980948)
									sender(id, "Готово<br>Айди: " + str(photo_list[0]["id"]))
								...
								if art_c[1] == 'кусь':
									...
									upload = VkUpload(vk_session_u)
									ph = [filename]
									photo_list = upload.photo(photos=ph, album_id=278594132, group_id=201980948)
									sender(id, "Готово<br>Айди: " + str(photo_list[0]["id"]))
								...
								if art_c[1] == 'выпить':
									...
									upload = VkUpload(vk_session_u)
									ph = [filename]
									photo_list = upload.photo(photos=ph, album_id=278594266, group_id=201980948)
									sender(id, "Готово<br>Айди: " + str(photo_list[0]["id"]))
								...
								if art_c[1] == 'трахнуть':
									...
									upload = VkUpload(vk_session_u)
									ph = [filename]
									photo_list = upload.photo(photos=ph, album_id=278597872, group_id=201980948)
									sender(id, "Готово<br>Айди: " + str(photo_list[0]["id"]))

					if "что с ботом" in event.obj.text.lower():
						send(id, "Кто-то: что с ботом?<br>Дима:", "photo-201980948_457239911")
					...
					if "заметка+" in event.obj.text.lower():
						if te2xt[0] == "заметка+":
							with open('notes.json', encoding='utf-8') as f:
								data = json.load(f)

							if str(te2xt[1]) == "1":
								data["note-1"] = msgl.split(' ', 2)[2]
							if str(te2xt[1]) == "2":
								data["note-2"] = msgl.split(' ', 2)[2]
							if str(te2xt[1]) == "3":
								data["note-3"] = msgl.split(' ', 2)[2]
							if str(te2xt[1]) == "4":
								data["note-4"] = msgl.split(' ', 2)[2]
							if str(te2xt[1]) == "5":
								data["note-5"] = msgl.split(' ', 2)[2]
							
							with open('notes.json', 'w', encoding='utf-8') as f:
								json.dump(data, f, ensure_ascii=False, indent=4)

							sender(id, "Готово!<br>Заметка " + msgl.split(' ', 2)[2] + " добавлена в " + str(te2xt[1]) + " слот")
					...
					if "заметка-" in event.obj.text.lower():
						if te2xt[0] == "заметка-":
							...
							sender(id, "Готово!<br>Заметка " + str(te2xt[1]) + " удалена")
